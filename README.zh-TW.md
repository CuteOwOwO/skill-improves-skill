...
`skill-generalizer` 幫助同一份 skill 更適合多個模型。它先觀察全新模型的實際行為，再加入一條有證據的最小修改；只有當每個目標都通過封存重測，而且沒有其他行為退步時，修改才會保留。
...
→ 找出一個重複缺口
→ 加入一條小 lesson
→ 用全新模型重測 source / candidate
→ 採用或退回
```

老師不會問模型「你想要什麼 prompt」，而是提供接近真實工作的情境，並依事先寫好的標準評分模型的決定、行動與輸出。

最後產出的是一份 common `SKILL.md`，不是每個模型各自一套提示詞。
...
在一次 Sol／Terra／Luna 小型測試中，一條新指令讓目標行為從 0/3 提升到 2/3，但 Luna 仍然漏掉它，並在另一題退步，因此補丁被退回。這不能證明模型之間存在穩定差異；它只說明為什麼需要重測閘門。
